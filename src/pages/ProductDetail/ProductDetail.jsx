...
import { catalogProducts } from '../../data/products.js';
...
export default function ProductDetail() {
  ...
  if (!product) return <Navigate to="/produkty" replace />;
    ...
    <main className={styles.page}>
      <section className={styles.hero}>
        <div className={styles.heroInner}>
          <div className={styles.breadcrumbs}><Link to="/produkty">Produkty</Link><span>/</span><span>{product.name}</span></div>
          <div className={styles.heroGrid}>
            <div className={`${styles.mainImage} ${product.id === 'm3' ? styles.productCarousel : ''}`}>
              <img key={productImages[activeSlide]} src={productImages[activeSlide]} alt={`${product.name}, zdjęcie ${activeSlide + 1}`} />
              {productImages.length > 1 && (
                <>
                  <button type="button" className={`${styles.carouselArrow} ${styles.arrowLeft}`} onClick={showPrevious} aria-label="Poprzednie zdjęcie"><span /></button>
                  <button type="button" className={`${styles.carouselArrow} ${styles.arrowRight}`} onClick={showNext} aria-label="Następne zdjęcie"><span /></button>
                  <div className={styles.carouselDots} aria-hidden="true">{productImages.map((image, index) => <span key={image} className={index === activeSlide ? styles.activeDot : ''} />)}</div>
                </>
              )}
            </div>
            <div className={styles.summary}>
              ...
              <dl>
                <div><dt>Numer katalogowy</dt><dd>{product.id.toUpperCase()}</dd></div>
                <div><dt>Rozmiar</dt><dd>{product.size}</dd></div>
                <div><dt>Waga</dt><dd>{weights[product.id]}</dd></div>
                <div><dt>Kolor</dt><dd>{product.color}</dd></div>
              </dl>
              <Link className={styles.inquiry} to="/kontakt">Zapytaj o produkt</Link>
            </div>
          </div>
        </div>
      </section>
      ...
      <section className={styles.features}>
        <div className={styles.sectionHeading}><span>Najważniejsze informacje</span><h2>Cechy produktu</h2></div>
      ...
      <section className={styles.material}>
        <div><span>Skład i wykonanie</span><h2>Krótko o materiale</h2></div>
        <p>{materialInfo[product.category]}</p>
      </section>

      <section className={styles.gallery}>
        <div className={styles.sectionHeading}><span>Produkt z bliska</span><h2>Galeria</h2></div>
        <div className={`${styles.galleryGrid} ${galleryImages.length === 2 ? styles.twoImageGallery : ''} ${galleryImages.length === 4 ? styles.fourImageGallery : ''}`}>
          {galleryImages.map((image, index) => (
            <div key={image} className={index === 0 && galleryImages.length === 3 ? styles.galleryLarge : ''}>
              <img src={image} alt={`${product.name}, zdjęcie galerii ${index + 1}`} />
            </div>
          ))}
        </div>
      </section>
